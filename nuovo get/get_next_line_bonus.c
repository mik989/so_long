...
/*   Updated: 2022/11/10 14:32:29 by mgirardi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*stringa[FOPEN_MAX];
	char		*linea;

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	stringa[fd] = ft_leggi_file(stringa[fd], fd);
	linea = ft_prima_linea(stringa[fd]);
	stringa[fd] = ft_prossima_linea(stringa[fd]);
	return (linea);
}
